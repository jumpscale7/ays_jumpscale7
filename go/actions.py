...
class Actions(ActionsBase):
    ...
    def configure(self,serviceobject):
        ...
        """
        gopath = serviceobject.hrd.get("instance.gopath")
        ...
        j.actions.start(name="create GOPATH", description='create GOPATH', action=createENV,  die=True, stdOutput=True, serviceObj=serviceobject)

        def installGodep():
            j.do.execute("/opt/go/bin/go get -u github.com/tools/godep")
        j.actions.start(name="install godep", description='install godep (can take a while)', action=installGodep,  die=True, stdOutput=True, serviceObj=serviceobject)
        return True
